  ...
  points: [
    {
      ...
      explain: [
        "官方课程的第一步是「引入全局样式」：它把全局样式表放在 `app/ui/` 目录下，文件名取作 global.css，然后在根布局里导入它。之所以挂在**根布局**，是因为根布局作用于全站的每个页面 —— 导入一次，所有路由都会带上这些样式。",
        "**本项目的这一步已经做好了，只是文件名和位置跟官方不一样**：本项目用的是 `app/globals.css`（Next.js 脚手架自带，Tailwind v4 的主题也写在里面），`app/layout.tsx` 顶部已经导入了它。你不需要动手，只要看懂这条链路。",
        "⚠️ 顺带说明一个容易踩的点：本项目里**没有** app/ui/global.css 这个文件（那是官方 starter 的放法）。看到官方文档提到它时，在本项目请对应到 `app/globals.css`，不要去项目里找一个不存在的文件。",
    ...
    },
    {
      ...
      pitfalls: [
        ...
        "页面文件必须**默认导出**一个组件（`export default function Page() {...}`）。如果只在文件里贴一段裸 JSX（比如单独一个 `<div />`），会报错：`Property 'default' is missing`（类型检查）或 JSX 语法错误 —— 这是最常见的「照抄示例代码」踩坑，示例里的片段要放进组件的 `return (...)` 里。",
      ],
    },
    {
      ...
      explain: [
        ...
        "用法：在 `app/playground/page.tsx` 顶部导入同目录下的样式模块（写成 `import styles from \"./pg.module.css\"`），然后 `<div className={styles.shape}>`。你写类名时不用关心会不会撞名，构建工具替你保证唯一。",
        "顺带一提：官方课程 starter 里本来就带了一个现成的 CSS Modules 示例（首页用的那个样式模块），本项目没有把它搬过来 —— 我们用不到它。所以这一条**就由你自己写一遍**：在练习页旁边新建 `app/playground/pg.module.css`，把刚才那个三角形用 CSS 重新实现。写完你会发现，Tailwind 和 CSS Modules 可以混用，没有谁替代谁。",
      ],
      code:
