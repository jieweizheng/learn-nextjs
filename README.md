...
2. **点某章的「学习计划」**，进入详情页：有学习目标、知识点地图，以及带**具体讲解**的知识点清单
   ...
   讲解默认全部展开；**给某条打勾后它会自动收起**，视线自然落到下一条。
   右上角还有「全部展开 / 全部收起讲解」，想通读或速览都方便。
...
- **点路径本身** → 用你的编辑器打开该文件（打开的是你本机项目目录里的真实文件）；
- **点后面的 ⧉** → 复制该文件的绝对路径。
...
（例如「在 Cursor 中打开：/path/to/learn-nextjs/app/lib/definitions.ts」）。
...
`NEXT_PUBLIC_PROJECT_ROOT=/path/to/learn-nextjs`（指定项目根目录）。
当然，**点 ⧉ 复制路径**这条路永远有效。
